...
<div>
    <div v-if="loadedData">
        <form novalidate class="md-layout md-alignment-center-center " md-elevation="4" @submit.prevent="can_We_Apply()">
                <md-card class="md-layout-item md-size-50 md-small-size-200 mycenter">
                    ...
                    <md-card-content>
                      <div>CL:{{leaves.Casual_Leave}}&nbsp;&nbsp;&nbsp;&nbsp;SL:{{leaves.Sick_Leave}}&nbsp;&nbsp;&nbsp;&nbsp;EL:{{leaves.Earned_Leave}}</div>
                       ...
                       <p v-if="form.selectedFromDate">{{form.selectedFromDate}}</p>
...
updateCasual(){
    ...
    console.log("in updateLeaveCount")
        db.collection("Leaves").doc(this.leaves.collection_Id).update({
                               
                                Casual_Leave:this.testleavecheck,
  ...
  updateSick(){
      db.collection("Leaves").doc(this.leaves.collection_Id).update({
                                ...
                                Sick_Leave:this.testleavecheck,
...
updateEarned(){
    db.collection("Leaves").doc(this.leaves.collection_Id).update({
                                ...
                                Earned_Leave:this.testleavecheck
                            }).then(()=>{
                             this.submit_Leave_Request ()
                            }).catch(error=>{
                      console.log(error.message)
...
can_We_Apply(){
 ...
 if("Casual"===this.form.type_of_leave){
      ...
      {
        this.testleavecheck=this.testleavecheck-this.dayDifferece
  ...
  else if("Sick"===this.form.type_of_leave){
      console.log("in first if")
      this.testleavecheck=this.leaves.Sick_Leave
      if((this.testleavecheck-this.dayDifferece)>=0)
      {
        this.testleavecheck=this.testleavecheck-this.dayDifferece
        console.log("in check")
  ...
  else if("Earned"===this.form.type_of_leave){
      console.log("in first if")
      this.testleavecheck=this.leaves.Earned_Leave
      if((this.testleavecheck-this.dayDifferece)>=0)
      {
        this.testleavecheck=this.testleavecheck-this.dayDifferece
        console.log("in check")
      ...
      submit_Leave_Request () {
        ...
        db.collection("Leave_requests").add({
                        ...
                        console.log("From:",this.form.selectedFromDate)
                        console.log("To:",this.form.selectedToDate)
